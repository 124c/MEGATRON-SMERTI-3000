...
data = moex_data(tickers=['SBER'], start_date=start_time, end_date=end_time, data_form='Close')
data_increments = data.ffill().pct_change().dropna()
data_increments.columns = ['close']
...
indicator_signals = alpha.deploy_alpha_engine(data_increments, params_dict=params_dict)
...
pnl_data, conf_matrix, hit_ratio = backtest.deploy_backtesting_engine(data=data_increments,
